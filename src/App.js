...
function wildAnswer() {
  if (
    userQuestion.includes(
      'world' || 'travel' || 'plane' || 'globe' || 'airport' || 'go' || 'fly',
    )
  ) {
    return travelCard
  }
...
switch (randomRelationships) {
  ...
  case 1:
    relationshipCard =
      'You will never find love, go to a Ping Pong show tomorrow'
    break
  case 2:
